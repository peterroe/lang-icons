...
import { getIconByFile, getIconByFolder } from '../src/material'
// import { getIconByFile, getIconByFolder } from '../dist/material.js'

describe('Icon test', () => {
  it('fetch file icon', () => {
    const svgIcon = getIconByFile('vite.config.ts')
  ...
  it('fetch file icon', () => {
    const svgIcon = getIconByFile('index.ts')
    expect(svgIcon).toMatchInlineSnapshot(`
      "<svg xmlns=\\"http://www.w3.org/2000/svg\\" viewBox=\\"0 0 32 32\\">
  ...
  it('fetch folder icon', () => {
    const svgIcon = getIconByFolder('src')
    expect(svgIcon).toMatchInlineSnapshot(`
      "<svg xmlns=\\"http://www.w3.org/2000/svg\\" viewBox=\\"0 0 32 32\\">
  ...
  it('fetch folder icon', async () => {
    const svgIcon = await getIconByFile('foo')
  ...
  it('fetch folder icon', async () => {
    const svgIcon = await getIconByFolder('foo')
    expect(svgIcon).toMatchInlineSnapshot('"<svg xmlns=\\"http://www.w3.org/2000/svg\\" viewBox=\\"0 0 24 24\\"><path fill=\\"#8699a3\\" d=\\"M4 20q-.825 0-1.412-.587T2 18V6q0-.825.588-1.412T4 4h6l2 2h8q.825 0 1.413.588T22 8v10q0 .825-.587 1.413T20 20z\\"/></svg>"')
  })
})
